...
public class HomeEventScheduler {
    ...
    public static void register() {
        ...
        ServerWorldEvents.LOAD.register((server, world) -> {
            ...
            if (world.getRegistryKey() == World.OVERWORLD) {
                ...
                if (!state.hasTimer(TIMER_ID)) {
                    ...
                    HorrorMod129.LOGGER
                            .info("HomeEventScheduler initialized with timer: " + state.getTimer(TIMER_ID) + " ticks");
                }
            }
        });
        ...
        ServerPlayConnectionEvents.DISCONNECT.register((handler, server) -> {
            ...
            HorrorMod129.LOGGER
                    .info("Player " + player.getName().getString() + " disconnected, time recorded: " + currentTime);
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            if (logoutTime > 0) {
                ...
                // Only trigger if event is ready AND server was empty AND player has been gone
                // long enough
        ...
        if (currentTimer > 0) {
            ...
            if (currentTimer == 0) {
                ...
                HorrorMod129.LOGGER.info(
                        "HomeEventScheduler timer reached zero, event is now ready for next extended log off or away period.");
            }
        }
            ...
            for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                if (player.getSpawnPointPosition() != null) {
                    ...
                    if (!isChunkLoaded && wasLoaded) {
                        ...
                        HorrorMod129.LOGGER.info(
                                "Player " + player.getName().getString() + "'s home chunk unloaded at " + currentTime);
                        ...
                        if (unloadTime > 0) {
                            ...
                            if (unloadDuration >= MIN_HOME_UNLOAD_TIME && eventReady) {
                                ...
                                if (!state.hasTimer(countdownKey)) {
                                    ...
                                    HorrorMod129.LOGGER.info("Player " + player.getName().getString() +
                                            "'s home chunk reloaded after " + unloadDuration
                                            + " seconds. Starting 3 second countdown.");
                                }
                            } else {
                                HorrorMod129.LOGGER.info("Player " + player.getName().getString() +
